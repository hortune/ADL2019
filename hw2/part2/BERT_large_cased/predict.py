...
def parse_args():
    ...
    parser.add_argument('input_csv', type=str, help='Inference batch size')
...
def predict(device, data_loader, max_sent_len, elmo_embedder, model):
    ...
    with torch.no_grad():
        ...
        for batch in bar:
            Ids += batch['Id']
            text_bert_word = batch['text_bert_word'].to(device=device)
            
            text_pad_mask = batch['text_pad_mask'].to(device=device)
            logits = model(text_bert_word, attention_mask = text_pad_mask).view(-1,5)
