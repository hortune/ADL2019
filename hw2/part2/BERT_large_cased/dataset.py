...
class Part1Dataset(Dataset):
    def __init__(self, data, word_vocab, char_vocab):
        ...
        self._data = [{
            ...
            'text_bert_word': tokenizer.convert_tokens_to_ids(d['text']),
            'label': int(d['label']) - 1
...
def create_collate_fn(word_vocab, char_vocab, max_sent_len, max_word_len):
    ...
    tokenizer = BertTokenizer.from_pretrained('bert-large-cased')
    bert_pad_idx = tokenizer.convert_tokens_to_ids(['[PAD]'])[0]
    ...
    def collate_fn(batch):
        ...
        text_bert_word = [b['text_bert_word'] for b in batch]
        ...
        text_bert_word = pad(text_bert_word, max_len, bert_pad_idx)
        ...
        text_bert_word = torch.tensor(text_bert_word)
        text_pad_mask = text_bert_word != bert_pad_idx
