...
class Model(BaseModel):
    def _create_net_and_optim(self, word_vocab, char_vocab, net_cfg, optim_cfg, train=True):
        #net = BCN(word_vocab, char_vocab, **net_cfg)
        #BertConfig
        net = BertForSequenceClassification.from_pretrained('bert-large-cased', num_labels=5)
        if train:
...
class Trainer(BaseTrainer):
    ...
    def _run_batch(self, batch):
        ...
        text_bert_word = batch['text_bert_word'].to(device=self._device)
        
        
        #if self._elmo_embedder and self._elmo_embedder.ctx_emb_dim > 0:
        #    text_ctx_emb = self._elmo_embedder(batch['text_bert_word'], self._max_sent_len)
        ...
        logits = self._model(text_bert_word, attention_mask=text_pad_mask).view(-1, 5)
