...
class Discriminator(nn.Module):
    def __init__(self,FeatureDim,hidden_size=32):
        ...
        """
        self.netD2 = nn.Sequential(
            nn.Linear(self.ndf*16*4*4+15, 512),
            nn.ReLU(True),
            nn.Linear(512,1)
        )
        self.apply(weights_init)
    ...
    def forward(self, inputs, embedding):
        ...
        x = torch.cat([imageC.view(inputs.size(0),-1),embedding],1)
        x = F.sigmoid(self.netD2(x))
        return x #, imageC
...
class DCGAN:
    ...
    def update_D(self, right_images, right_embed, wrong_embed, *argvs):
        self.D.zero_grad()
        # real real
        real_loss = self.D(right_images, right_embed)
        wrong_loss = self.D(right_images, wrong_embed)
        ...
        errD_fake = self.D(fake, right_embed)
    ...
    def update_G(self, right_images, right_embed, wrong_embed, *argvs):
        self.G.zero_grad()
        noise = torch.bernoulli(
                    torch.empty(right_images.size(0), 
                                self.noise_dim).
