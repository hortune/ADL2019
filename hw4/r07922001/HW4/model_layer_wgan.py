...
class Discriminator(nn.Module):
    def __init__(self,FeatureDim, adv_loss, hidden_size=64):
        ...
        self.conv5 = nn.Conv2d(self.ndf*8, self.ndf*1, 4, 2, 1, bias=False)
        
        self.netD = nn.Sequential(
                    self.conv1,
                    nn.LayerNorm([self.ndf, 64, 64], elementwise_affine=False),
                    nn.LeakyReLU(0.2, True),
                    self.conv2,
                    nn.LayerNorm([self.ndf*2, 32, 32], elementwise_affine=False),
                    nn.LeakyReLU(0.2, True),
                    self.conv3,
                    nn.LayerNorm([self.ndf*4, 16, 16], elementwise_affine=False),
                    nn.LeakyReLU(0.2, True),
                    self.conv4,
                    nn.LayerNorm([self.ndf*8, 8, 8], elementwise_affine=False),
                    nn.LeakyReLU(0.2, True),
                    self.conv5,
                    nn.LayerNorm([self.ndf, 4, 4], elementwise_affine=False),
                )
