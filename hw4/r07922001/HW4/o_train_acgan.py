...
D = Discriminator(FeatureDim, 64).cuda()
...
fixNoise = torch.randn(4,NoiseDim).cuda().detach()
...
def compute_GP(netD, real_data, real_embed, fake_data, LAMBDA):
        ...
        alpha = alpha.expand(BATCH_SIZE, int(real_data.nelement() / BATCH_SIZE)).view(BATCH_SIZE, 3, 128, 128)
        alpha = alpha.cuda()
    ...
    for epoch in range(500):
        ...
        while i < len(t2IDLoader):
            ...
            while j < Diters:
                ...
                D.zero_grad()
                # real real
                real_loss, cls_real = D(right_images)
                
                # train with fake image
                noise = torch.randn(right_images.size(0),NoiseDim).cuda()
                fake = G(right_embed,noise).data
                errD_fake, cls_fake = D(fake)
                gp =  compute_GP(D, right_images, right_embed, fake, 5)
            ...
            if gen_iterations % 500 == 0:
                fake = G(evaluate_embed[:4], fixNoise).mul(255)
